...
class manageproductController extends Controller
{
    public function index()
    {
        ...
        return view('manageproduct', compact('products','categorys'));
    }

    public function changeInfo(request $request, $id){
        $productedit = Product::findOrFail($id);
        $productedit->stock = $request->input('stock');
        $productedit->save();
    ...
    public function addProduct(request $request){
        ...
        if($request->has("name") && $request->has("desc") && $request->has("cat") && $request->has("price") && $request->has("stock") && $request->hasFile("image")){
            ...
            $name = str_replace($a, $b, $product->name);
            $image = $request->file('image');
            $filename = $name . '.' . $image->getClientOriginalExtension();
            $image->move('public/pictures/product',$filename);

            $pict = Picture::create([
                'url' => 'public/pictures/product/'.$filename
            ]);

            $product->pictures()->attach($pict);

            $cat = Category::where("name","=",$request->input("cat"))->first();
